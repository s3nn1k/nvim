...
- Hack Nerd Font (brew cask) - обязателен (иконки web-devicons, диагностические знаки);
  ставится через `./init.sh`, он же применяет шрифт к профилю iTerm2 по умолчанию
...
4. Каждая внешняя зависимость (CLI, LSP, форматтер) прописывается в списке Зависимости и в
   `init.sh` одновременно; установка - только через `./init.sh`.
